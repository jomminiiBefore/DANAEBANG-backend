...
class RoomFindTest(TestCase):
    maxDiff = None
    def setUp(self):
        roomtype = RoomType.objects.create(name = '원룸')
        TradeType.objects.create(
            id = 1,
            name = '월세'
        )
    ...
    def test_find_room_get_success(self):
        client   = Client()
        response = client.get('/room/list?latitude=37.505776&longitude=127.052472&zoom=10&offset=1&limit=24&multi_room_type=1&selling_type=2&room_size=0&room_size=100&maintenance_price=0&maintenance_price=10&deposit_range=0&deposit_range=20000')
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json(),
                         {
                             "results":[
            ...
            "https://d2o59jgeq8ig2.cloudfront.net/complex/default/complex_default_detail2.png",
                                     ...
                                     "floor_str" : "1층",
                                     "room_size" : "45.44",
                                     "latitude" : "37.587519000000000",
                                     "longitude" : "127.075277999999997",
                                     "maintenance_price" : 10,
                                     "trade_type_str" : "전세",
                                     "trade_deposit" : 10000,
                                     "monthly_type_str" : "월세",
    ...
    def test_find_room_post_success(self):
        client   = Client()
        rooms = {"room_id_list" : [1]}
        response = client.post('/room/list?offset=1&limit=24', json.dumps(rooms), content_type = 'application/json')
        self.assertEqual(response.status_code, 200)

    def test_find_room_invalid_query_parameter(self):
        client   = Client()
        response = client.get('/room/list?&longitude=127.052472&zoom=10&offset=1&limit=24&multi_room_type=1&selling_type=2&room_size=0&room_size=100&maintenance_price=0&maintenance_price=10&deposit_range=0&deposit_range=20000')
